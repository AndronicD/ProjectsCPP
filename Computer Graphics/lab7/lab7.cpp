...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int light_pos_loc = glGetUniformLocation(shader->GetProgramID(), "light_position");
    glUniform3fv(light_pos_loc, 1, glm::value_ptr(lightPosition));

    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform
    int eye_position_loc = glGetUniformLocation(shader->GetProgramID(), "eye_position");
    glUniform3fv(eye_position_loc, 1, glm::value_ptr(eyePosition));

    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    int  material_kd_loc = glGetUniformLocation(shader->GetProgramID(), "material_kd");
    glUniform1f(material_kd_loc, materialKd);
    int material_ks_loc = glGetUniformLocation(shader->GetProgramID(), "material_ks");
    glUniform1f(material_ks_loc, materialKs);
    int material_shininess_loc = glGetUniformLocation(shader->GetProgramID(), "material_shininess");
    glUniform1i(material_shininess_loc, materialShininess);
    int object_color_loc = glGetUniformLocation(shader->GetProgramID(), "object_color");
